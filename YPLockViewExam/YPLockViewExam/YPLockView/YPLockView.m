...
+ (instancetype)didFinishPathBlock:(YPLockViewDidFinishPathBlock)didFinishPathBlock
{
    YPLockView *lockView = [self lockViewWithType:YPLockViewTypeThree];
    lockView.didFinishPathBlock = didFinishPathBlock;
    return lockView;
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([self.delegate respondsToSelector:@selector(lockView:didFinishPath:)]) {
        [self.delegate lockView:self didFinishPath:[self getPassword]];
    }
    // block
    if (self.didFinishPathBlock) {
        self.didFinishPathBlock(self,[self getPassword]);
    }
